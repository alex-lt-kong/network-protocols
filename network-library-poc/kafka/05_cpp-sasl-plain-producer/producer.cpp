...
int main(int argc, char **argv) {
  ...
  string brokers = "172.16.0.2:9092,172.16.0.2:9093,172.16.0.2:9094";
  string topic   = "ak-topic";  
  string errstr;
  ...
  if (conf->set("security.protocol", "SASL_PLAINTEXT", errstr) != Conf::CONF_OK) {
    cerr << errstr << std::endl;
    exit(1);
  }
  if (conf->set("sasl.mechanism", "PLAIN", errstr) != Conf::CONF_OK) {
    cerr << errstr << std::endl;
    exit(1);
  }
  /*
  username and password here should be consistent with the definition in
  kafka_server_jaas.conf
  */
  if (conf->set("sasl.username", "admin1", errstr) != Conf::CONF_OK) {
    cerr << errstr << std::endl;
    exit(1);
  }
  if (conf->set("sasl.password", "password666", errstr) != Conf::CONF_OK) {
    cerr << errstr << std::endl;
    exit(1);
  }
