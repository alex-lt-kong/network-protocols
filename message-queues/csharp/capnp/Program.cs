...
namespace MyAddressBook
{
    class Program
    {
        ...
        static Person[] generateRandomData() {
            ...
            for (int i = 0; i < TEST_SIZE; ++i)
            {
                ...
                persons[i] = new Person()
                {
                    ...
                    {
                        new Person.PhoneNumber() { TheType = Person.PhoneNumber.Type.work, Number = (uint)rnd.Next(0, 2147483647) }
                    },
        ...
        static MemoryStream encodeStructToBytesCapnp(Person person, MessageBuilder mb, Person.WRITER root) {
            MemoryStream mem = new MemoryStream();            
            person.serialize(root);;
            FramePump pump = new FramePump(mem);
            pump.Send(mb.Frame);
        ...
        static void Main(string[] args)
        {
            Random rnd = new Random(Guid.NewGuid().GetHashCode());
            Person[] persons = new Person[TEST_SIZE];
            Person.READER[] person_readers = new Person.READER[TEST_SIZE];
            MemoryStream[] mems = new MemoryStream[TEST_SIZE];
            Console.WriteLine("===== Testing CapnProto =====");
            ...
            for (int i = 0; i < TEST_SIZE; ++i)
            {
                ...
                Person.WRITER root = mb.BuildRoot<Person.WRITER>();
                mems[i] = encodeStructToBytesCapnp(persons[i], mb, root);
            }
            ...
            for (int i = 0; i < TEST_SIZE; ++i)
            {
                WireFrame frame = Framing.ReadSegments(mems[i]);
                DeserializerState deserializer = DeserializerState.CreateRoot(frame);
                person_readers[i] = new Person.READER(deserializer);
            }
            ...
            Person.READER rnd_person_reader = person_readers[rnd.Next(0, (int)TEST_SIZE-1)];
            Console.WriteLine($"Id: {rnd_person_reader.Id}");
            Console.WriteLine($"Name: {rnd_person_reader.Name}");
            Console.WriteLine($"E-mail: {rnd_person_reader.Email}");
            Console.WriteLine($"Self-intro: {rnd_person_reader.SelfIntroduction}");
            if (rnd_person_reader.Phones?.Count > 0)
            {
                foreach (var phone in rnd_person_reader.Phones)
                {
            ...
            for (int i = 0; i < 16; ++i) {
                Console.Write(rnd_person_reader.Scores[i]);
                Console.Write(", ");
            }
            Console.WriteLine("]");
