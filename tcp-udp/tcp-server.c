...
int main() {
  ...
  while (1) {
    ...
    printf("[%s] Connection accept()'ed. server: %s:%d <-> client %s:%d\n",
           get_iso_datetime(iso_dt), inet_ntoa(addr.sin_addr),
           ntohs(addr.sin_port), inet_ntoa(client_addr.sin_addr),
           ntohs(client_addr.sin_port));

    /* write()/send() and read()/recv() are almost idenitical except that send()
     * and recv() receive an extra paramter flag, giving us more flexibility
     */
    do {
      // recv() blocks the loop until any data is readable.
      bytes = recv(new_socket_fd, buffer, READ_BUF_SIZE, 0);
      if (bytes > 0) {
